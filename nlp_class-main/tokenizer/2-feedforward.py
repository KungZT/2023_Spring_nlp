...
if __name__ == '__main__':
    ...
    config = Config()
    voc_size = len(config.vocab)#词汇表大小
    ...
    for lin in config.train_data:
        ...
        train_data_list.append(torch.tensor(hang, dtype=torch.long))#遍历训练数据中的每一行，将每个字符转换为其在词汇表中的索引，并将索引列表转换为 PyTorch 的张量。
    ...
    for lin in config.target:
        ...
        target_list.append(torch.tensor(hang, dtype=torch.long))#将标签字符映射到数字标签，并将每个标签序列转换为 PyTorch 的张量。
    ...
    rec = []
    prec = []
    f1 = []#记录训练过程中损失和性能指标的列表
    optimizer = torch.optim.SGD(model.parameters(), lr=config.learning_rate)# 随机梯度下降 (SGD) 优化器，并传入模型的参数和学习率。
    ...
    for i in tqdm(range(config.epoch)):
        for j, k in enumerate(train_data_list):
            ...
            loss.backward()
            optimizer.step()#前向传播计算输出、计算损失、反向传播更新参数。
